...
add_to_path() {
  ...
  path_line='export PATH="$HOME/.subweazl/bin:$PATH"'
  marker_begin="# >>> subweazl path >>>"
  marker_end="# <<< subweazl path <<<"
...
(
  cd "$REPO_ROOT"
  GOCACHE="$GO_CACHE" GOMODCACHE="$GO_MOD_CACHE" go build -buildvcs=false -o "$BIN_PATH" ./cmd/subweazl
)
...
echo '  export PATH="$HOME/.subweazl/bin:$PATH"'

if [[ "${SUBWEAZL_SKIP_LAUNCH:-}" == "1" ]]; then
  echo "Skipping first launch because SUBWEAZL_SKIP_LAUNCH=1"
else
  echo ""
