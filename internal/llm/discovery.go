...
func FetchProviderModels(ctx context.Context, providerType, serverURL string) ([]string, error) {
	...
	case "vllm":
		...
		if err := getDiscoveryJSON(ctx, serverURL+"/v1/models", &body); err != nil {
			return nil, err
		}
	...
	case "ollama":
		...
		if err := getDiscoveryJSON(ctx, serverURL+"/api/tags", &body); err != nil {
			return nil, err
		}
...
func ProviderPaths(providerType string) (chat, models string) {
	if providerType == "ollama" {
		return "/api/chat", "/api/tags"
	}
